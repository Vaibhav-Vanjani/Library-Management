...
const EARNING_REDIRECT_TO = "/admin/earning";
const SEARCH_STUDENT_REDIRECT_TO = "/admin/searchStudent";
...
export default function () {
  ...
  useEffect(() => {
    async function getDefaulter() {
      try {
        const response = await fetch(process.env.REACT_APP_BACKEND_URL + "/admin" + "/defaulter", {
          method: "GET",
    ...
    const interval = setInterval(async function (params) {
      try {
        const response = await fetch(process.env.REACT_APP_BACKEND_URL + "/admin" + "/api/check-scan/",{
          method:"GET",
          headers:{"Content-Type":"application/json"},
          credentials:"include",
        });
  ...
  const button_list = [
    {
      cardName: PENDING_PAYMENT_CARD_NAME,
      goto: PENDING_PAYMENT_REDIRECT_TO,
      props: { pendingDefaulter },
    },
    {
      cardName: ENROLLMENT_FORM_CARD_NAME,
      goto: ENROLLMENT_FORM_REDIRECT_TO,
      props: {},
    },
    { cardName: ENTRY_EXIT_CARD_NAME, goto: ENTRY_EXIT_REDIRECT_TO, props: {} },
    {
      cardName: SEARCH_STUDENT_CARD_NAME,
      goto: SEARCH_STUDENT_REDIRECT_TO,
      props: {},
    },
  ];
